...
export function WeightLab() {
  ...
  const field = (key: keyof CombinationInput, label: string, optional = false) => <label>{label}{optional && ' (valfri)'}<div className="number"><input type="number" min="0" inputMode="numeric" value={data[key] ?? ''} onChange={(e) => setData((old) => ({ ...old, [key]: optional && e.target.value === '' ? undefined : Number(e.target.value) }))}/><span>kg</span></div></label>;
  ...
  return <section className="panel"><span className="eyebrow">Interaktiv övning</span><h2>Viktlabbet</h2><p>Ändra värdena och se skillnaden mellan <strong>körkortsbehörighet</strong> och <strong>bilens tekniska gränser</strong>.</p>
    <div className="weight-grid">
      <div className="weight-card"><h3>🚙 Dragbil</h3>{field('carTotalKg','Totalvikt')}{field('carGrossKg','Aktuell bruttovikt')}{field('carMaxTrailerGrossKg','Högsta släpvagnsvikt (O.1)')}{field('carMaxTrainGrossKg','Högsta tågvikt (F.3)', true)}</div>
    ...
    <SourceLinks ids={['TS-SLAP','TS-SLAP-REGLER','TS-VIKTER-SLAP','TS-BE']} />
  </section>;
}
