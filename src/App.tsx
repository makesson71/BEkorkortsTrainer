...
type View = 'home' | 'diagnostic' | 'practice' | 'lessons' | 'weights' | 'sources';
...
export default function App() {
  ...
  const home = <>
    ...
    <section><div className="section-head"><div><span className="eyebrow">Träna smart</span><h2>Fyra vägar in</h2></div></div><div className="cards">
      <button className="feature-card" onClick={() => setView('diagnostic')}><span>🧠</span><h3>30-årsdiagnos</h3><p>Hitta vad som faktiskt behöver fräschas upp.</p></button>
      <button className="feature-card" onClick={() => setView('weights')}><span>⚖️</span><h3>Viktlabbet</h3><p>Flytta siffrorna och förstå B, B96, BE och tekniska gränser.</p></button>
      <button className="feature-card" onClick={() => setView('lessons')}><span>📚</span><h3>Snabblektioner</h3><p>3–8 minuter per område, alltid med myndighetskälla.</p></button>
      <button className="feature-card" onClick={() => setView('practice')}><span>🎯</span><h3>Frågeträning</h3><p>Vet, tror eller gissar – så rätt gissning inte lurar systemet.</p></button>
    </div></section>
    <section className="notice"><strong>Inte en officiell Trafikverket-app.</strong> Alla träningsfrågor är egenformulerade. Regelfakta ska vara spårbara till Trafikverket eller Transportstyrelsen och ha datum för senaste kontroll.</section>
  </>;

  return <div className="app"><header><button className="brand" onClick={() => setView('home')}><span>BE</span><div>Trainer<small>Sverige</small></div></button><nav><button onClick={() => setView('lessons')}>Lär</button><button onClick={() => setView('weights')}>Vikter</button><button onClick={() => setView('practice')}>Träna</button><button onClick={() => setView('sources')}>Källor</button></nav></header><main>
    {view === 'home' && home}
